...
async def edit_cb(cb: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if action == 'edit_contents':
        ...
        await cb.message.delete()
        if 'name' in action:
            msg = _("Текущее имя: <code>{box_name}</code>{sn}Введите новое").format(box_name=box.name, sn='\n')
        else:
            msg = _("Текущее место: <code>{box_place}</code>{sn}Введите новое").format(box_place=box.place, sn='\n')
        ...
        await cb.message.answer(f"<code>{content.name}</code>\n" +
                                _("нажми, чтобы скопировать и отправь мне исправленное"))
...
async def delete_callback_handler(cb: types.CallbackQuery, callback_data: dict):
    ...
    if action == 'delete_content':
        ...
        await cb.message.edit_text(
            _("Вы уверены что хотите {sn}удалить ящик № {box_id}").format(sn='\n', box_id=box_id),
